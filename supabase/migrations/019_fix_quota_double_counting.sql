/**
 * Migration: 019_fix_quota_double_counting
 *
 * 目的：修復配額雙重計數問題
 *
 * 問題描述：
 * - Migration 015 的 create_listing_safe() 函數在第 136-139 行手動更新配額 +1
 * - Migration 018 的觸發器在 AFTER INSERT 時又自動更新配額 +1
 * - 導致每次創建刊登，配額增加 2 而不是 1
 *
 * 解決方案：
 * - 重新建立 create_listing_safe() 函數，移除手動配額更新邏輯
 * - 完全依賴 Migration 018 的 sync_user_quota_on_listing_change() 觸發器自動管理配額
 * - 保留配額檢查邏輯，確保不超過上限
 */
...
-- ============================================================
-- 2. 建立新版本的 create_listing_safe() 函數（移除配額更新邏輯）
-- ============================================================
...
BEGIN
  ...
  -- 步驟 5: 返回結果
  -- ========================================
  -- ⚠️  移除配額更新邏輯：Migration 018 的觸發器會自動處理
...
COMMENT ON FUNCTION create_listing_safe IS
  '安全建立刊登：使用悲觀鎖確保配額檢查和刊登建立的原子性。配額更新由 Migration 018 觸發器自動管理。';
