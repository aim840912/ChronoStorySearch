...
**本文件已於 2025-10-26 拆分為模組化文件以便閱讀和維護。**

### 📖 推薦閱讀方式

**請前往新的模組化文件** → **[交易系統架構設計 (模組化版本)](./交易系統/README.md)**
...
- **[開始閱讀新版文件](./交易系統/README.md)** ← 推薦
- **[實作路線圖 (情境 A: 新專案)](./交易系統/08-實作路線圖.md#情境-a從零開始建立系統)**
- **[實作路線圖 (情境 B: 現有系統)](./交易系統/08-實作路線圖.md#情境-b優化現有系統已上線)**

---

> **說明**: 本檔案保留作為歷史記錄,內容與新版文件完全一致。
>
> **建議**: 使用新的模組化文件以獲得更好的閱讀體驗。
...
1. [系統概述](#系統概述)
2. [系統架構](#系統架構)
3. [Discord OAuth 2.0 認證](#discord-oauth-20-認證)
4. [資料庫設計](#資料庫設計)
5. [API 設計](#api-設計)
6. [Discord Webhook 通知](#discord-webhook-通知)
7. [Discord Bot 伺服器整合](#discord-bot-伺服器整合)
8. [信譽系統](#信譽系統)
9. [安全性設計](#安全性設計)
10. [可靠性設計](#可靠性設計)
11. [效能優化](#效能優化)
...
Discord OAuth 2.0 使用標準的 Authorization Code Grant 流程，提供安全的第三方登入機制。

**流程步驟**：
1. 用戶點擊「使用 Discord 登入」
2. 重導向至 Discord 授權頁面（帶 state 參數防 CSRF）
3. 用戶在 Discord 授權應用
4. Discord 重導向回應用（帶 code 和 state）
5. 後端用 code 交換 access_token 和 refresh_token
6. 用 access_token 獲取 Discord 用戶資料
7. 創建/更新資料庫用戶記錄
8. 設置 session cookie

### Discord Application 設置

前往 [Discord Developer Portal](https://discord.com/developers/applications) 創建應用：

```yaml
Application Name: MapleStory Trading System
Description: 虛擬物品交易意向媒合平台

OAuth2 配置:
  Client ID: <從 Discord 獲取>
  Client Secret: <從 Discord 獲取，存入環境變數>
  ...
  OAuth2 Scopes:
    - identify        # 獲取用戶基本資訊（ID, username, avatar）
    # email 可選，若需要郵件通知功能可啟用
```

**⚠️ Scopes 說明**：
- ✅ **identify**：**必須**。獲取 Discord ID、用戶名、頭像等基本資訊
- ⚠️ **email**：**可選**。若需要郵件通知可啟用，但增加用戶授權摩擦
- ❌ **guilds.members.read**：**不推薦**。檢查用戶是否在特定伺服器，實作複雜且不必要

**建議配置**：
- **最小化權限**：僅使用 `identify`（推薦起步階段）
- **簡化授權流程**：減少用戶疑慮，提高轉換率
- **後續擴展**：待有明確需求時再增加 scopes
...
1. **State 參數驗證**：防止 CSRF 攻擊
   - 產生隨機 state 並存入 JWT
   - JWT 設置 10 分鐘過期時間
   - 回調時驗證 state 一致性和未過期

2. **Token 加密存儲**：
   - Discord access_token 和 refresh_token 使用 AES-256-GCM 加密
   - 加密密鑰來自環境變數 SESSION_SECRET
...
```
users (用戶表)
  ├── 1:N → sessions (Session 表)
  ├── 1:1 → discord_profiles (Discord 詳細資料)
  ├── 1:N → reputation_history (信譽歷史)
  ├── 1:N → listings (刊登表)
  ├── 1:N → interests (購買意向表 - 作為買家)
  ├── 1:N → reports (舉報表 - 作為舉報者)
  └── 1:1 → user_quotas (配額表)
...
- **Token 加密存儲**：AES-256-GCM 加密
- **Session Cookie 安全**：HttpOnly + Secure + SameSite

### 2. API 安全

- **Rate Limiting**：防止濫用（10K 請求/天）
...
**風險場景**：
- 🔴 **XSS 攻擊竊取 Cookie**：雖然使用 HttpOnly，但仍需防範 XSS
- 🔴 **CSRF 攻擊**：偽造請求利用已登入的 session
- 🔴 **中間人攻擊**：HTTP 連接被竊聽
...
async function getUser(username: string) {
  ...
  // 若 username = "admin' OR '1'='1"，則會返回所有用戶！
...
async function getUser(username: string) {
  const { data } = await supabase
    .from('users')
    .select('*')
    .eq('username', username) // 自動參數化，防止 SQL Injection
    .single()
...
async function getUserWithRawSQL(username: string) {
  const { data } = await supabase.rpc('get_user_by_username', {
    p_username: username // 參數傳遞，而非字串拼接
  })
...
BEGIN
  ...
  FROM users u
  WHERE u.discord_username = p_username; -- 參數化，安全
END;
...
**Session 儲存方式**：
- 使用 **HTTP-only Cookie** 儲存加密的 JWT token
- Token 包含：user_id、discord_id、過期時間、簽發時間
...
const SESSION_CONFIG = {
  ...
  COOKIE_MAX_AGE: 30 * 24 * 60 * 60, // 30 天
  REMEMBER_ME_EXPIRES_IN: 90 * 24 * 60 * 60, // 90 天（可選功能）
}
...
**自動刷新時機**：
- Token 剩餘有效期 < 25%（例如：7 天 token，剩餘 < 1.75 天）
- 用戶活躍時才刷新（避免無效刷新）
...
export function useSession() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      if (!response.ok) {
        // Session 過期，觸發登入 Modal（見 DDR-003）
        const event = new CustomEvent('show-login-modal')
        window.dispatchEvent(event)
      }

      // 如果 response 包含新的 Set-Cookie header，瀏覽器會自動更新
    }, 5 * 60 * 1000) // 5 分鐘
...
export async function revokeAllUserSessions(userId: string): Promise<void> {
  ...
  // 2. 在 session 驗證時檢查此標記
}

// 修改 validateSession 函數
export async function validateSession(
  request: NextRequest
): Promise<{ valid: boolean; user?: User }> {
  // ... 原有邏輯 ...
...
CREATE TABLE user_sessions (
  ...
  -- 裝置資訊
  device_name TEXT,  -- 從 User-Agent 解析
  device_type TEXT,  -- 'mobile' | 'desktop' | 'tablet'
  browser TEXT,      -- 'Chrome' | 'Firefox' | 'Safari'
  os TEXT,           -- 'Windows' | 'macOS' | 'Linux' | 'iOS' | 'Android'

  -- Session 資訊
  session_token_hash TEXT NOT NULL UNIQUE,  -- Token 的 SHA-256 hash
  ip_address INET,
  last_active_at TIMESTAMPTZ DEFAULT NOW(),
  expires_at TIMESTAMPTZ NOT NULL,
...
- [ ] 建立 Supabase 專案（Free tier）
- [ ] 安裝依賴：`npm install @supabase/supabase-js`
- [ ] 建立資料庫表：`users`, `sessions`, `listings`, `interests`
- [ ] 設定 RLS 策略

**Week 3-4：環境配置與基礎工具**
- [ ] 安裝 Redis：`npm install @upstash/redis`
- [ ] 建立 Supabase 客戶端工具
- [ ] 設定環境變數（`.env.local`）
- [ ] 建立基本的 API route 測試連線
...
**Week 5-6：OAuth 核心流程**
- [ ] 建立 Discord Application（Developer Portal）
- [ ] 實作 OAuth 啟動：`GET /api/auth/discord`
- [ ] 實作 OAuth 回調：`GET /api/auth/discord/callback`
- [ ] State token CSRF 防護
...
**Week 9-10：刊登系統**
- [ ] 實作刊登 CRUD API
- [ ] 實作認證中間件（`withAuthAndError`）
- [ ] 建立刊登 Modal 元件（`CreateListingModal`）
  - 重用 `BaseModal` 基礎元件
  - 表單驗證與錯誤處理
...
- [ ] 實作市場列表 API
- [ ] 實作購買意向 API
- [ ] 建立市場瀏覽 Modal（`MarketBrowserModal`）
  - 商品列表展示（含分頁/篩選）
  - 刊登詳情子 Modal（嵌套顯示）
  - 購買意向登記功能
...
### DDR-001：選擇 Discord OAuth 而非角色 ID 系統

**日期**：2025-10-25

**背景**：原系統使用角色 ID + Token 的無密碼認證，但存在以下問題：
- 無法驗證用戶真實身份
- 容易被濫用（多帳號註冊）
- 缺乏信任機制

**決策**：改用 Discord OAuth 2.0 認證

**理由**：
- ✅ 真實身份驗證（綁定 Discord ID）
- ✅ 防止多帳號濫用（一個 Discord ID 只能綁定一個帳號）
- ✅ 信譽系統基礎（Discord 帳號年齡可驗證）
- ✅ 免費且無使用限制
- ✅ 用戶基礎龐大（Discord 在遊戲社群普及）

**權衡**：
- ❌ 需要用戶有 Discord 帳號（但目標用戶群體已廣泛使用 Discord）
- ❌ OAuth 流程較複雜（但提供更好的安全性）

**結果**：
- 提升系統安全性
- 建立信譽系統基礎
- 整合 Discord 生態系統（Webhook、Bot）
...
**背景**：交易系統 UI 有兩種實作方式可選：
1. **Modal 模式**：使用 `BaseModal` 在主頁上疊加顯示
2. **獨立頁面**：建立 `/market`、`/listings/[id]` 等路由

**決策**：採用 Modal 模式實作交易系統 UI

**理由**：

1. **符合專案現有架構** ✅
   - 專案已實作完整的 Modal 基礎建設（`BaseModal`、`ModalManager`）
   - 現有所有功能均採用 Modal 模式（ItemModal、MonsterModal、GachaMachineModal 等）
   - 保持 UI 一致性，降低用戶學習成本

2. **節省 Vercel 資源消耗** ✅
   - 減少 Serverless Functions 執行時間：~2 小時/月（節省 2% 額度）
   - 降低 Bandwidth 消耗：~0.3 GB/月（節省 0.3% 額度）
   - 總體節省約 3% Vercel 免費額度

3. **開發效率更高** ✅
   - 重用現有 `BaseModal` 元件（無需建立路由架構）
   - 重用現有狀態管理邏輯（`useModalManager`）
   - 開發時間預估節省 30-40%
...
1. ❌ **空間限制**
   - Modal 可視範圍受限（max-w-6xl），不適合複雜介面
   - 解決方案：MVP 階段保持功能簡潔，必要時可擴展為獨立頁面

2. ❌ **巢狀 Modal 複雜度**
   - 「市場列表 → 刊登詳情 → 登記意向」可能需要多層 Modal
   - 解決方案：使用 `hasPreviousModal` 和 `onGoBack` 實作導航邏輯

3. ❌ **無獨立 URL**
   - 無法直接分享特定刊登連結
   - 解決方案：未來可透過 URL hash 參數實作（如 `/#listing-12345`）

**擴展性考量**：

保留混合架構的彈性：
- **MVP 階段**（Week 9-12）：完全使用 Modal 模式
- **優化階段**（可選）：若發現以下情況，可擴展為獨立頁面
  - 賣家需要管理大量刊登（> 10 個）
  - 需要複雜的市場分析介面
  - 需要 SEO 友善的市場頁面
...
**結果**：
- 符合專案一致性原則（重用現有架構）
- 節省 Vercel 成本（Function Time -67%，Bandwidth -5%）
- 加快 MVP 開發速度（預估節省 1-2 週）
- 保留未來擴展為獨立頁面的選項

**參考資料**：
- [成本預估 - Vercel Functions 執行時間分析](#vercel-functions-執行時間分析)
- [實作路線圖 - 階段 2：核心功能](#階段-2核心功能第-9-12-週)
...
**背景**：用戶需要登入才能使用刊登和購買意向功能。當 API 返回 401 Unauthorized 時，需要引導用戶登入。

有兩種實作選擇：
1. **LoginModal**：在主頁上疊加 Modal 顯示登入按鈕
2. **登入頁面**：獨立的 `/auth/login` 路由頁面

**決策**：採用 LoginModal

**理由**：

1. **符合 DDR-002 Modal 優先原則** ✅
   - 與交易系統 UI（CreateListingModal、MarketBrowserModal）保持一致
   - 整個應用統一使用 Modal 模式
   - 降低用戶學習成本

2. **更好的用戶體驗** ✅
   - 用戶停留在主頁上下文（不中斷瀏覽流程）
   - 清晰的登入說明和 Discord OAuth 流程提示
   - 視覺化的載入狀態（防止用戶困惑於跳轉過程）
   - 即時回饋（錯誤訊息可以在 Modal 內顯示）

3. **符合架構要求** ✅
   - 滿足「載入狀態處理」需求（Line 4213）
   - 滿足「錯誤訊息顯示」需求（Line 4214）
   - 滿足「使用 Discord 登入按鈕」需求（Line 4212）
   - 重用 BaseModal 基礎建設
...
1. ❌ **需要全局狀態管理**
   - Modal 需要在任何地方都能觸發（當 API 返回 401 時）
   - 解決方案：使用 CustomEvent 實作全局觸發機制

2. ❌ **無獨立 URL**
   - 無法直接分享「登入頁面」連結
   - 解決方案：用戶需要登入時自動顯示 Modal（體驗更流暢）
...
export function LoginModal({ isOpen, onClose }: LoginModalProps) {
  ...
  return (
    <BaseModal
      ...
      onClose={onClose}
      maxWidth="max-w-md"
      preventBackdropClose={isLoading}  // 載入時防止關閉
    >
...
**視覺設計**：
- 使用 Discord 品牌色 `#5865F2`（主要按鈕）
- Hover 狀態：`#4752C4`（稍深）
- 載入狀態：顯示 spinner + 「跳轉中...」文字
- 禁用狀態：50% 透明度 + 禁用指標

**結果**：
- 符合 Modal 優先設計原則（與 DDR-002 一致）
- 提供完整的登入 UX（說明、載入、錯誤處理）
- 保持應用 UI 一致性（所有功能均使用 Modal）
- 節省開發時間（重用 BaseModal 基礎建設）

**參考資料**：
- [DDR-002：選擇 Modal 模式而非獨立頁面](#ddr-002選擇-modal-模式而非獨立頁面)
- [實作路線圖 - 階段 1：Discord OAuth 認證](#階段-1discord-oauth-認證第-5-8-週)
