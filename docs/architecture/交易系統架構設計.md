...
> **⚠️ 重要更新**：基於實際流量分析（7,794 訪客/5天，60-80% Bot 流量），Redis 已超標 107%。
>
> **調整策略**：優先實作 Bot Detection（緊急），再實作 Discord OAuth（重要）
...
| **階段 3** | 第 10-13 週 | Discord OAuth 實作 | - | 🟢 High Priority |
| **階段 4** | 第 14-17 週 | Webhook & 信譽系統 | - | 🟡 Medium |
| **階段 5** | 第 18+ 週 | Discord Bot & 增強功能 | - | ⚪ Optional |

**時程摘要**：
- **最小可行版本（MVP）**：5 週（階段 0+1，解決 Redis 超標）
- **穩定運行版本**：9 週（階段 0+1+2，完整 Bot 防護）
- **完整功能版本**：17 週（所有階段，含 Discord OAuth + Webhook）

---

### 階段 0：緊急修復 - Bot Detection 階段 1（第 1 週）

**目標**：臨時緩解 Redis 超標問題，降至 70-80% 使用率

**優先級**：🔴 **Critical**（Redis 當前 107% 超標，系統可能被限流）

#### 任務清單

**Week 1 - Day 1-2：基礎過濾實作**
- [ ] 建立 Bot Detection 模組結構
  - 檔案：`src/lib/middleware/bot-detection.ts`
  - 功能：User-Agent 黑名單檢查
- [ ] 實作 SEO 爬蟲白名單
  - 允許：GoogleBot、BingBot、BaiduSpider 等
- [ ] 撰寫單元測試
  - 測試各種 User-Agent 字串
  - 邊界條件測試（空值、特殊字符）

**Week 1 - Day 3-4：Middleware 整合**
- [ ] 建立 Next.js Middleware
...
**Week 1 - Day 5-7：監控與驗證**
- [ ] 實作 Redis 使用量監控
  - 檔案：`src/lib/monitoring/redis-monitor.ts`
  - 每小時記錄命令數
- [ ] 部署到 Staging 環境
  - 驗證 Bot 過濾效果
  - 確認真實用戶不受影響
- [ ] 上線到 Production
  - 監控 24 小時
  - 確認 Redis 使用率降至 70-80%

#### 驗收標準

✅ **功能驗收**：
- User-Agent 黑名單正確過濾（curl、wget、python-requests 等）
- SEO 爬蟲白名單正常工作（GoogleBot 通過）
- 真實瀏覽器請求不受影響（Chrome、Firefox、Safari 通過）

✅ **效能驗收**：
- Redis 命令數降至 7,000-8,000/天（70-80% 使用率）
- Middleware 處理時間 < 5ms
- 無明顯延遲或錯誤

⚠️ **風險提示**：
- 過於激進的過濾可能誤殺真實用戶（需謹慎測試）
- 階段 1 僅為臨時緩解，仍接近上限（需盡快實作階段 2）

---

### 階段 1：穩定基礎 - Bot Detection 階段 2（第 2-5 週）

**目標**：實作行為模式檢測，降至 50-60% Redis 使用率，確保系統穩定運行

**優先級**：🔴 **Critical**（確保免費版可長期穩定運行）
...
**5 週 MVP**（推薦起步階段）：
```
Week 1：Bot Detection 階段 1（緊急修復）
Week 2-5：Bot Detection 階段 2（穩定基礎）
→ 結果：Redis 使用率降至 50-60%，系統可穩定運行
```
...
**17 週完整版**（包含所有核心功能）：
```
Week 1-9：Bot Detection 四階段（完整防護）
Week 10-13：Discord OAuth（認證系統）
Week 14-17：Webhook & 信譽（增強功能）
→ 結果：生產級交易平台，所有核心功能完備
```
...
| 里程碑 | 時間點 | 成果 |
|--------|--------|------|
| 🔴 **緊急修復** | Week 1 | Redis 降至 70-80%，臨時緩解 |
| 🟢 **穩定運行** | Week 5 | Redis 降至 50-60%，可長期運行 |
| 🎯 **理想狀態** | Week 9 | Redis 降至 20-30%，完整防護 |
| ✅ **核心功能** | Week 13 | Discord OAuth 上線 |
| 🚀 **完整功能** | Week 17 | 所有功能完備 |
