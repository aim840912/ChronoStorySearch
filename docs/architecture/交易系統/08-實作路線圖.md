...
> **最後更新**：2025-10-29（完成度：80% - Bot Detection 階段 1+2 完成）
...
**Week 2：高頻訪問檢測** ✅
- [x] 實作 Rate Limiter 服務
  - 檔案：`src/lib/bot-detection/rate-limiter.ts`
  - 功能：固定窗口算法（INCR + EXPIRE）
- [x] 設定端點級別限流
  - trending：30 次/小時（嚴格）
  - search：40 次/小時（中等）
  - listings：100 次/小時（認證用戶寬鬆）
- [x] 整合到 Middleware
  - 檢測並阻擋高頻請求
...
**Week 3：掃描行為檢測** ✅
- [x] 實作路徑多樣性檢測
  - 檔案：`src/lib/bot-detection/behavior-detector.ts`
  - 功能：追蹤 IP 訪問的端點數量
- [x] 設定掃描閾值
