...
machineIds.forEach((id) => {
  const filePath = path.join(gachaDir, `machine-${id}.json`)
  const data = JSON.parse(fs.readFileSync(filePath, 'utf8'))
  console.log(`Machine ${id}: ${data.items.length} 個物品`)
...
const uniqueItemIds = Array.from(allItemIds).sort((a, b) => a - b)

console.log(`\n找到 ${uniqueItemIds.length} 個獨特物品 ID`)
console.log('開始下載圖示...\n')
...
downloadAll().then((results) => {
  ...
  if (results.notFound.length > 0) {
    const missingListPath = path.join(outputDir, 'missing-gacha-icons.json')
    fs.writeFileSync(
      missingListPath,
      JSON.stringify(results.notFound, null, 2),
      'utf8'
    )
    console.log(`\n📝 缺失圖示清單已儲存: missing-gacha-icons.json`)
