...
function DropItemCard({ drop }: { drop: DropItem }) {
  const [itemImageError, setItemImageError] = useState(false)
  const chancePercent = (drop.chance * 100).toFixed(4)
  const qtyRange =
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4 border border-gray-200 dark:border-gray-600 hover:shadow-md transition-shadow">
      <div className="flex items-center gap-3 mb-3">
        {/* 物品圖示 */}
        {itemIconUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
        ...
        <div className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {monsterDrops.map((drop, index) => (
              <DropItemCard key={`${drop.itemId}-${index}`} drop={drop} />
            ))}
          </div>
        </div>
      </div>
...
export default function Home() {
  ...
  // 最愛怪物管理
  const { favorites, toggleFavorite, isFavorite, favoriteCount } = useFavoriteMonsters()
  ...
  const uniqueFavoriteMonsters = useMemo(() => {
    if (filterMode !== 'favorites' || favorites.length === 0) return []
  ...
  useEffect(() => {
    ...
    if (filterMode === 'favorites') {
      // 最愛模式：只顯示最愛怪物的掉落
      const favMobIds = new Set(favorites.map((fav) => fav.mobId))
      baseDrops = allDrops.filter((drop) => favMobIds.has(drop.mobId))
    } else {
      // 全部模式
